...
| `resources.ts` | `toggleResource`, `updateResource`, `uploadResourceFile`, `removeResourceFile` | Admin-only via shared `requireAdmin()`. Powers `/admin/resources` toggles + edit form; all revalidate `/resources`. `updateResource` also persists `external_url`. `uploadResourceFile`/`removeResourceFile` (service client) manage the file in the private `resource-files` bucket + the `resources` file columns. Public download/view flow through `GET /api/resources/download/[slug]` (signed URL or external redirect + `download` event + `increment_resource_download`) and `POST /api/resources/view` (validated `view` events + `increment_resource_view`), the latter fed by the `ResourceViewTracker` `sendBeacon` on `/resources`. |
...
| `blog.ts` | `createBlogPost`, `updateBlogPost`, `deleteBlogPost` | `requireAdmin()` guard; slug uniqueness enforced in both create + update. Persists `featured_image_path` + `featured_image_alt` (always written, so an explicit `NULL` clears the image); `sanitizeImageUrl()` rejects anything that isn't absolute `http(s)`. All three revalidate `/blog` and `/blog/[slug]`. Featured-image uploads go through `media.ts` → `uploadEditorImage` — there is no separate blog upload action |
...
- **Inline images/GIFs:** the toolbar image button opens the shared **`src/components/admin/MediaPicker.tsx`** (Upload / GIF search / Photo search / URL), which the newsletter editor and the blog **featured image** field both reuse — three call sites in total. Upload → `uploadEditorImage` (`src/app/actions/media.ts`) → public `blog-images` bucket under `inline/`. Search proxies (admin-gated, keys server-side, graceful when unset): `GET /api/media/gif` (Giphy, `GIPHY_API_KEY`) and `GET /api/media/image` (Unsplash, `UNSPLASH_ACCESS_KEY` + `trackUnsplashDownload` ping). Giphy/Unsplash URLs are hotlinked (their CDN); only uploaded files land in our bucket.
