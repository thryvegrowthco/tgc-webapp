...
| `(marketing)` | `/`, `/about`, `/services`, `/blog`, `/book`, `/consultation`, `/contact`, `/faq`, `/investment`, `/packages`, `/privacy`, `/resources`, `/career-reset-workbook` (lead-magnet landing page for the "Career Reset Workbook" resource — direct download via `/api/resources/download/career-reset-workbook` + optional `NewsletterForm source="career-reset-workbook"`), `/testimonials`, `/terms` | `Header` + `Footer` from `(marketing)/layout.tsx` | Public |
| _(no group)_ `src/app/card/` | `/card`, `/card/rachel-dietz.vcf` | Root layout only, no Header/Footer; the page draws its own cream and sage bands | **Public**; `robots` noindex; left out of `sitemap.ts` |

**Business card page (`src/app/card/`):** `/card` is the URL encoded in the QR code on Rachel's printed business cards (artwork lives outside this repo in `Desktop/Apps/designs/thryve-growth-co/business-card/`). Every card in circulation depends on it: change the page freely, but never delete or move the route. It replaced a 307 redirect to `/consultation` (PR #5), so `next.config.ts` no longer has `redirects()`; Next checks redirects before pages, so the two cannot coexist.
- `page.tsx`: standalone server component. Cream band: logo (`public/logos/FAVORITE 1 (2).png` via `next/image`) and tagline. Sage band: headshot (`public/images/headshots/rachel-card.jpg`, a 512×512 crop of `rachel94.jpg`), name, title, **Book a free consultation** → `/consultation` (deliberately no UTMs), **Save to contacts** → the vCard, and a `mailto:` link. White band: short bio, links to `/services/hr-consulting` and `/services/coaching`, LinkedIn and Instagram. The copy has no dashes (the owner's rule for anything tied to the card).
- Metadata: title "Rachel Dietz" (the root template adds the brand); `robots: { index: false, follow: true }` (QR traffic, and the rest of the site never publishes Rachel's surname); `openGraph`/`twitter` share title and description; a file-based `opengraph-image.jpg` + `opengraph-image.alt.txt` (generated by the card folder's `tools/build_mockups.mjs`) so a shared link previews the card. Left out of `sitemap.ts` but not disallowed in `robots.ts`, so crawlers can read the noindex.
- `src/lib/card/contact.ts`: `CARD_CONTACT` (the same copy as the printed card) and `VCARD_PATH`; `buildVCard(photoJpegBase64?)` returns vCard 3.0 with CRLF line endings, RFC 2426 escaping and UTF-8-safe folding at 75 octets.
- `card/rachel-dietz.vcf/route.ts`: `export const dynamic = "force-static"`, so it is prerendered at build and reads the headshot with `readFile(join(process.cwd(), "public/images/headshots/rachel-card.jpg"))` then, never at request time; the photo is embedded as `PHOTO`. Headers: `Content-Type: text/vcard; charset=utf-8`, `Content-Disposition: attachment; filename="Rachel Dietz.vcf"`, `X-Robots-Tag: noindex`.
- Live contract: the card folder's `node build.mjs` fetches `https://www.thryvegrowth.co/card` and refuses to write print files unless it answers 200 with no redirect and still contains `Rachel Dietz`, `href="/consultation"`, `mailto:rachel@thryvegrowth.co` and `href="/card/rachel-dietz.vcf"` (`card.json` → `qr.expect.contains`).
- Attribution: nothing in the app stores UTMs. `/card` page views in Vercel Analytics are the scan count; a consultation request that starts here is stored like any other (`leads.source = "consultation"`).
...
**Admin pages:** `/admin` (overview, now with a "Top tasks" widget above Recent Bookings), `/admin/tasks` (filter by upcoming/overdue/completed), `/admin/notifications` (full inbox), `/admin/leads` (+ `/admin/leads/[id]`, with a "Create proposal" button → `/admin/proposals/new?leadId=` and a list of that lead's proposals), `/admin/proposals` (list) (+ `/admin/proposals/new` accepting `?leadId=` / `?clientId=`, and `/admin/proposals/[id]` — edit, locked once accepted/paid/declined), `/admin/sessions` (session queue with status filter + client/service search), `/admin/invitations` (+ `/admin/invitations/new?clientId=` — booking-invitation builder; `BookingInvitationForm` adapts the `BulkSlotForm` date/time pattern), `/admin/bookings`, `/admin/clients` (+ `/admin/clients/new` — admin-created accounts, no payment; + `/admin/clients/[id]`, with a "Job Alerts access" panel (`WatchlistAdminControls`, works with or without a `watchlist_profiles` row), an "Account not activated" banner with `ResendInviteButton` when `auth.users.email_confirmed_at` is null, per-client Tasks panel, a "Create booking invitation" button, a "Proposals" panel with a "+ New proposal" link → `/admin/proposals/new?clientId=`, and a Phase 3 "Goals" panel — `client_goals` and `watchlist_profiles` added to the page's parallel `Promise.all`, rendered via `GoalsManager` in `adminMode`), `/admin/content` (blog), `/admin/resources` (+ `/admin/resources/[id]` — toggles + edit form for the `/resources` catalog), `/admin/testimonials` (Phase 3 — moderation queue with all statuses grouped pending → approved → hidden + counts; + `/admin/testimonials/[id]` edit and `/admin/testimonials/new` manual add), `/admin/watchlists` (+ `/admin/watchlists/[clientId]`), `/admin/analytics`. The "Proposals" nav item sits between Invitations and Bookings, and the "Testimonials" item sits after Resources, in `AdminNav` (`src/components/admin/AdminNav.tsx`).
...
> `AUTH_ROUTES` includes `/reset-password`, so an **authenticated** user is bounced off it to `/dashboard`. Since `verifyOtp` creates a session, any email link that lands there never shows a password field — and `/reset-password` is only the "email me a link" *request* form anyway. Password-setting lives on `/dashboard/profile`; both the recovery hook and the client-invite flow target it.

**Auth email links (`/auth/confirm`).** `src/app/auth/confirm/route.ts` handles `?token_hash=` links (the PKCE `?code=` flow is `/auth/callback`) and redirects to `origin + next`. `next` arrives from the Send Email hook as `redirect_to`, which Supabase forwards **verbatim** from `emailRedirectTo` — usually an absolute URL. Concatenating that yields e.g. `https://thryvegrowth.co` + `https://thryvegrowth.co/auth/callback`, which parses *without throwing* to the garbage host `thryvegrowth.cohttps` and sends the user nowhere. Both `/auth/confirm` (`safeNextPath`) and `/api/auth/send-email` (`toPath`) now reduce same-origin absolutes to a path and reject everything else, which doubles as an open-redirect guard. `/auth/confirm` accepts `signup | recovery | email_change | invite | magiclink`.
...
| `resources.ts` | `createResource`, `updateResource`, `toggleResource`, `deleteResource`, `finalizeResourceFile`, `removeResourceFile` | Admin-only via shared `requireAdmin()`. Powers `/admin/resources` (list + `new` create page + `[id]` editor); all revalidate `/resources`. `createResource` auto-slugs the title (unique) and inserts hidden (`enabled=false`); `deleteResource` removes the row + its stored file. `updateResource` persists `external_url`. **Files upload directly to Storage** (`POST /api/admin/uploads/sign` → browser `PUT`, bypassing the Server Action 1 MB / Vercel 4.5 MB body caps); `finalizeResourceFile` then records `file_path/file_name/file_size_bytes`; `removeResourceFile` clears them + deletes the object. Public download/view flow through `GET /api/resources/download/[slug]` (signed URL or external redirect + `download` event + `increment_resource_download`) and `POST /api/resources/view` (validated `view` events + `increment_resource_view`), the latter fed by the `ResourceViewTracker` `sendBeacon` on `/resources`. |
...
| `blog.ts` | `createBlogPost`, `updateBlogPost`, `deleteBlogPost` | `requireAdmin()` guard; slug uniqueness enforced in both create + update. Persists `featured_image_path` + `featured_image_alt` (always written, so an explicit `NULL` clears the image); `sanitizeImageUrl()` rejects anything that isn't absolute `http(s)`. All three revalidate `/blog` and `/blog/[slug]`. Featured-image uploads go through the `MediaPicker` (direct signed-URL upload) — there is no separate blog upload action |
| `media.ts` | `trackUnsplashDownload` | `requireAdmin()` guard. Pings a picked photo's Unsplash `download_location` (best-effort, no-store). **Image uploads no longer live here** — the `MediaPicker` uploads directly to Storage via `POST /api/admin/uploads/sign` (see below). |
| `watchlist.ts` | Client: `saveWatchlistProfile`, `updateMatchStatus`, `toggleFavorite`, `updateMatchNotes`, `updateApplicationDetails`. Admin: `addManualJob`, `assignJobToClient(clientId, jobId, curation?)`, `toggleRachelRecommended`, `removeJobMatch`, `fetchJSearchJobsForClient`, `runAutoMatchForClient`, `updateWatchlistProfileAsAdmin`, `setWatchlistReviewStatus`, `pauseWatchlist`, `reactivateWatchlist`, `cancelWatchlist`, `grantComplimentaryAccess`, `revokeComplimentaryAccess`, `toggleJobSource` | Client + admin actions in one file; each has its own auth check. Save/assign/fetch emit `client_notifications` + emails (`new_job_match`/`curated_job_match`/`watchlist_updated`). `pause/reactivate/cancel` act on the Stripe subscription + local status, and now refuse on a comped row (pointing at revoke instead — they're Stripe verbs). `grant/revokeComplimentaryAccess` are thin `requireAdmin()` wrappers over `src/lib/watchlist/comp.ts`; **grant upserts**, so it works for a client with no watchlist row (`setSubscriptionStatus` is UPDATE-only and silently no-ops there). `toggleJobSource` flips `job_sources.enabled`. Auto-match uses `src/lib/matching/score.ts`, inserting only matches with score ≥ 60 (excluded-employer / unmet must-have force exclusion). |
| `clients.ts` | `createClientAccount`, `resendClientInvite` | Admin-created client accounts — onboard someone with no payment and no card. Both use `service.auth.admin.generateLink` (**not** `inviteUserByEmail`, which 422s `email_exists` and therefore cannot resend; links expire in ~24h so resend is mandatory). `type: 'invite'` creates the `auth.users` row; the `handle_new_user()` trigger writes `profiles` (`role='client'`, `full_name` from `options.data`), and a follow-up update adds `phone`. The invite link is built by hand with a **relative** `next` (`/dashboard/profile?welcome=1`) and emailed via `sendClientInvite`, so it bypasses the Send Email hook entirely. Duplicate emails are pre-checked against `profiles` and returned as `{ error, existingClientId }` so the form can link to the existing client. `sendInvite` is optional — an account can be created silently and invited later. `resendClientInvite` uses `type: 'magiclink'` (invite 422s for an existing user) and refuses once `email_confirmed_at` is set. |
...
- **Inline images/GIFs:** the toolbar image button opens the shared **`src/components/admin/MediaPicker.tsx`** (Upload / GIF search / Photo search / URL), which the newsletter editor and the blog **featured image** field both reuse — three call sites in total. **Upload = direct-to-storage** via `uploadViaSignedUrl("blog-images", …)` (`src/lib/upload/direct.ts` → `POST /api/admin/uploads/sign` → browser `PUT`), which bypasses the Server Action 1 MB / Vercel 4.5 MB body caps; the returned public URL is inserted. Search proxies (admin-gated, keys server-side, graceful when unset): `GET /api/media/gif` (Giphy, `GIPHY_API_KEY`) and `GET /api/media/image` (Unsplash, `UNSPLASH_ACCESS_KEY` + `trackUnsplashDownload` ping). Giphy/Unsplash URLs are hotlinked (their CDN); only uploaded files land in our bucket.

- **Direct uploads:** `POST /api/admin/uploads/sign` (`requireAdmin`, bucket allowlist `{resource-files, blog-images}`, ≤25 MB) mints a Supabase `createSignedUploadUrl`; the browser `PUT`s the file straight to Storage. Client helper: `uploadViaSignedUrl(bucket, file, prefix?)`. Callers: `MediaPicker` (blog-images) and `ResourceEditForm` (resource-files → `finalizeResourceFile`). This replaced the old FormData Server Actions, which threw past ~1 MB and left the UI stuck on "Uploading…".
...
- **Featured images:** chosen through the same `MediaPicker` (`BlogPostForm.tsx`), so a cover image is either an upload to the **public `blog-images`** bucket under `inline/` — served via `getPublicUrl()`, a permanent public URL required so images also render in newsletter inboxes — or a hotlinked Unsplash CDN URL. The picked alt text is stored in `blog_posts.featured_image_alt` and rendered on the blog index card, the post hero, and `openGraph.images[].alt`, falling back to the post title when `NULL`. Client documents use the private `documents` bucket with signed URLs instead.
  - All blog images render with a plain `<img>`, not `next/image` — `next.config.ts` declares no `remotePatterns`, so a hotlinked `images.unsplash.com` URL would throw if piped through `<Image>`.
...
| `/api/cron/job-feed` | `0 13 * * *` (daily 1 PM UTC) | Automated multi-source ingest. Authenticates, reads `JOB_FEED_BATCH` (default 5) least-recently-fed active clients via the `watchlist_profiles.last_feed_at` cursor, then answers `202 Accepted` immediately and runs the ingest in `after()` (`maxDuration = 300`) — cron-job.org drops any request after 30 s, so the work cannot sit on the response. Per client: `getEnabledSources()` → `ingestForClient` (fetch → dedup → score → assign → `new_job_match` notify) → stamps `last_feed_at`. Writes exactly one `job_feed_run` row to `automation_log` (payload includes `durationMs`); a run with errors also emails Rachel via `sendAdminAlert`. A failed pre-flight DB read returns `500` so cron-job.org still alerts. Idempotent. |
...
| `/api/cron/expire-matches` | `0 12 * * *` (daily) | For active watchlist clients, flips `new`/`saved`/`interested` matches to `expired` when the posting's `closes_at` has passed, or (no deadline) when `date_posted` is older than `EXPIRE_AFTER_DAYS` (default 45). Filters status in TS (never `.eq`/`.in` on the union column); updates in chunks; logs an `expire_matches_run` row. Idempotent. Expired matches move to the Inactive tab (admin `?tab=inactive`, client `?view=inactive`). |
