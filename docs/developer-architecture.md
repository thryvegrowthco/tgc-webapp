...
**Redirects (`next.config.ts` → `redirects()`):** `/card` → `/consultation?utm_source=print&utm_medium=business_card&utm_campaign=rachel_card_2026`, `permanent: false` (307, not cached). This is the URL encoded in the QR code on Rachel's printed business cards (artwork lives outside this repo in `Desktop/Apps/designs/thryve-growth-co/business-card/`, whose build checks this redirect live before it will write print files). Cards in circulation depend on it: change the destination if needed, never remove the entry. Next applies redirects before the filesystem and proxy, and passes any incoming query string through. The UTM values only surface in analytics (Vercel Analytics and the consent-gated pixels); `/consultation` stores leads with a fixed `source` and does not persist them.
